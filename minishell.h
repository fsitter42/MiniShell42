...
/*   Updated: 2026/04/17 12:23:22 by slambert         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void							add_cmd_to_cmd_list(t_cmd **cmd_list,
									t_cmd *cmd);
...
t_token							*tokenlist_add(t_token *list_start, int type,
									char *str, int quote_status);
int								not_a_word(char c1, char c2);
int								b_validate_token_sequence(t_token *token,
									t_data *data);
...
int								consume_syntactic_quote(char c,
									int *quote_status);
int								expand_home_dir(t_token *list_elem,
									char **envp);
...
void							remove_implicit_null_arg(t_token **prev,
									t_token *next, t_token **list);
int								create_and_fill_new_tokens(char **split_result,
									t_token *list);
int								normalizer(char *str, char *ifs,
									char ***split_result);
int								cleanup_ifs_and_return(char *ifs,
									int ifs_is_allocated, int ret);
int								is_implicit_null_word(t_token *list);
int								replace_first_split_word(t_token *list,
									char **split_result);
...
// f_export_key.c
int								f_lst_add_one(t_envl *list, char *key,
									char *val);
...
char							**f_envp_lst_to_p(t_envl *list, size_t keylen,
									size_t vallen, size_t i);
...
// f_pathhandler.c
char							*f_path_handler(t_data *data, char *cmd,
									char **envp);
...
// f_exe.c
int								f_exec_pipeline(t_data *data, t_cmd *cmd,
									int pipe_fd[2]);
...
void							f_close_pipe(t_cmd *cmd, int pipe_fd[2],
									int *prev_fd);
void							f_close_child(int *pipe_fd, int prev_fd,
									t_cmd *cmd);
void							f_setup_pipe_fds(t_data *data, t_cmd *cmd,
									int *prev_fd, int *pipe_fd);
void							f_setup_cmd_fds(t_data *data, t_cmd *cmd,
									int *pipe_fd, int prev_fd);

// f_exec_error.c
int								f_pipe_error(t_data *data, t_cmd *cmd,
									int *prev_fd);
int								f_fork_error(t_data *data, t_cmd *cmd,
									int pipe_fd[2], int *prev_fd);
void							f_dup_error(t_data *data, t_cmd *cmd,
									int *pipe_fd, int prev_fd);
int								f_redir_error(int saved_fds[2], int fd,
									char *err);
